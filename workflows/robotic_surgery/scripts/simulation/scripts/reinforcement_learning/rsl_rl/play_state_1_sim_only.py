...
def main():
    ...
    # ===== OBSERVATION LOADING (DISABLED - using simulation observations only) =====
    # To enable loading observations from Excel file, see play_state_1_with_override.py
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # ===== NO OVERRIDE - using simulation observations only =====
            # To enable override from Excel file, see play_state_1_with_override.py
            # ============================================================
        ...
        if done_flag:
            ...
            # # ===== SAVE GALLBLADDER BINARY MASK AT FINAL STEP =====
            # try:
            ...
            #         camera_rgb = camera_sensor.data.output["rgb"][0, ..., :3].float().cpu().numpy()  # [H, W, C]
                    
            #         # Normalize to [0, 1] if needed
            #         if camera_rgb.max() > 1.0:
            #             camera_rgb = camera_rgb / 255.0
                    
            #         # Create mask using the gallbladder detection function
            ...
            #         mask_np = mask[0].cpu().numpy().astype(np.uint8) * 255  # Convert to 0-255
            ...
            #         print(f"[INFO] Final gallbladder mask saved (step {timestep}): {mask_path}")
            #         print(f"[INFO] Mask shape: {mask_np.shape}, unique values: {np.unique(mask_np)}")
            #     else:
            #         print("[WARNING] Camera sensor not available for final mask save")
            # except Exception as e:
            #     print(f"[WARNING] Could not save final mask at step {timestep}: {e}")
            # # ===== END FINAL MASK SAVE =====
