...
def main():
    ...
    sock_ack.bind(('0.0.0.0', ACK_PORT))
    sock_ack.settimeout(2.0)
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    REAL_OBS_PORT = 5006
    sock_recv_obs = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock_recv_obs.bind(('0.0.0.0', REAL_OBS_PORT))
    sock_recv_obs.settimeout(5.0)
    print(f"[INFO] Listening for real robot obs on port {REAL_OBS_PORT}")
   
    
    def receive_real_obs():
        try:
            data, _ = sock_recv_obs.recvfrom(1024)
            values = np.array(struct.unpack('16f', data), dtype=np.float32)
            return values[:8], values[8:16]
        except socket.timeout:
            print("[WARNING] Timeout ricevendo obs reali dal robot.")
            return None, None
    ...
    print("[INFO] Waiting for robot to be ready...")
    time.sleep(0.5)
    # Svuota il buffer di obs in arrivo durante l'attesa
    sock_recv_obs.settimeout(0.1)
    while True:
        try:
            sock_recv_obs.recvfrom(1024)
        except socket.timeout:
            break
    sock_recv_obs.settimeout(5.0)
    ...
    # ========== MANUAL JOINT RESET ==========
    # Override initial joint positions with manual values
    MANUAL_RESET_POS = torch.tensor([[0.9773, -0.3491, 0.080, 1.0, 0.0, 0.0]], device=agent_cfg.device)
    ...
    print("[INFO] Manual joint reset applied.")
    print("[INFO] Joint positions set to:", MANUAL_RESET_POS[0, :6].cpu().numpy())
    
    print("[INFO] Waiting for first UDP packet from robot before starting...")
    while True:
        try:
            data_obs, addr_obs = sock_recv_obs.recvfrom(1024)
            if len(data_obs) == 64:
                print("[INFO] First UDP packet received. Starting loop.")
                break
        except socket.timeout:
            print("[WARNING] Timeout waiting for first UDP packet, retrying...")
            continue
    # =========================================
    ...
    # ========================================
    print("[INFO] Waiting for first real obs...")
    real_pos_8, real_vel_8 = receive_real_obs()
    if real_pos_8 is not None:
        obs_sim_flat = obs.get("policy")[0].cpu().numpy()
        obs_modified = obs_sim_flat.copy()
        obs_modified[:8]  = real_pos_8
        obs_modified[8:16] = real_vel_8
        obs_tensor = torch.tensor(obs_modified, dtype=torch.float32, device=agent_cfg.device).unsqueeze(0)
        obs_before_real = TensorDict({"policy": obs_tensor}, batch_size=env.num_envs)
    else:
        obs_before_real = obs
        print("[WARNING] Usando obs simulata come fallback.")
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            obs_before = obs_before_real
            pos_before = robot_asset.data.joint_pos[0][:6].cpu().numpy()
            true_pos = robot_asset.data.joint_pos[0].cpu().numpy()  # solo per logging
            true_vel = robot_asset.data.joint_vel[0].cpu().numpy()  # solo per logging

            # === SCENARIO B: sostituisci pos e vel con dati dal robot reale ===
            real_pos_8, real_vel_8 = receive_real_obs()

            if real_pos_8 is None:
                # fallback: usa obs simulata se nessun dato arriva
                obs_before = obs
                print("[WARNING] Usando obs simulata come fallback.")
            else:
                # Estrai obs simulata flat per prendere target_pose[16:23] e last_action[23:29]
                if hasattr(obs, 'get'):
                    obs_sim_flat = obs.get("policy")[0].cpu().numpy()
                else:
                    obs_sim_flat = obs[0].cpu().numpy() if torch.is_tensor(obs) else np.array(obs)

                # Sostituisci pos[0:8] e vel[8:16] con dati reali, mantieni il resto dalla sim
                obs_modified = obs_sim_flat.copy()
                obs_modified[:8]   = real_pos_8   # joint pos dal robot reale
                obs_modified[8:16] = real_vel_8   # joint vel dal robot reale

                obs_tensor = torch.tensor(obs_modified, dtype=torch.float32,
                                          device=agent_cfg.device).unsqueeze(0)
                obs_before = TensorDict({"policy": obs_tensor}, batch_size=env.num_envs)
                print(f"[REAL OBS] pos[0:3]={real_pos_8[:3].round(4)} vel[0:3]={real_vel_8[:3].round(4)}")
            # ===================================================================
            ...
            if not done_flag:
                ...
                try:
                    ack, _ = sock_ack.recvfrom(1024)
                except socket.timeout:
                    print(f"[WARNING] No ACK from robot.")
