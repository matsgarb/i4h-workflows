...
def main():
    ...
    # sock_recv.settimeout(None) # change
    ...
    # ========== DATA SAVING SETUP ==========
    SAVE_DATA = True
    observations_data_list = []  # Collect observations for each timestep
    actions_data_list = []  # Collect actions/errors for each timestep
    if SAVE_DATA:
        print(f"[INFO] Data saving enabled. Two Excel files will be saved at episode end.")
    # ========================================
    ...
    # MANUAL_RESET_POS = torch.tensor([[-0.15, 0.0, 0.106, -0.35, 0.1, 0.1]], device=agent_cfg.device)
    # MANUAL_RESET_POS = torch.tensor([[-0.01, -0.3, 0.145, -1.4, -0.0256, 0.2216]], device=agent_cfg.device)
    MANUAL_RESET_POS = torch.tensor([[ 0.9773, -0.3491, 0.080, 1, 0, 0.0]], device=agent_cfg.device)
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            while True:
                try:
                    data, addr = sock_recv.recvfrom(1024, socket.MSG_DONTWAIT)
                    if len(data) == 64:
                        latest_data = data
            ...
            try:
                data_obs, addr_obs = sock_recv.recvfrom(1024)
                if len(data_obs) == 64:
                    unpacked_obs = struct.unpack('16f', latest_data)
            ...
            # ===== OBSERVATIONS DATA SAVING =====
            if SAVE_DATA:
                obs_row = {'timestep': timestep}
                # Joint positions (first 6 of the 8)
                for j in range(6):
                    obs_row[f'joint_pos_j{j}'] = joint_pos_obs[j]
                # Joint velocities (first 6 of the 8)
                for j in range(6):
                    obs_row[f'joint_vel_j{j}'] = joint_vel_obs[j]
                # Policy/actions (6 values)
                for j in range(6):
                    obs_row[f'policy_j{j}'] = actions_obs[j]
                observations_data_list.append(obs_row)
            # =====================================
            ...
            # ===== ACTIONS AND ERRORS DATA SAVING =====
            if SAVE_DATA:
                actions_row = {'timestep': timestep}
                # Position of joints (6 values)
                for j in range(6):
                    actions_row[f'position_j{j}'] = pos_after[j]
                # Raw policy (6 values)
                for j in range(6):
                    actions_row[f'raw_policy_j{j}'] = raw_policy[j]
                # Processed actions (6 values)
                for j in range(6):
                    actions_row[f'processed_actions_j{j}'] = processed_actions[j]
                # Real movement (6 values)
                for j in range(6):
                    actions_row[f'real_movement_j{j}'] = real_move[j]
                # Distance and orientation errors
                actions_row['distance_error'] = dist_error
                actions_row['orientation_error'] = orient_error
                actions_data_list.append(actions_row)
            # ==========================================
        ...
        if done_flag:
            ...
            # Save data to Excel files if enabled
            if SAVE_DATA:
                # Save observations data
                if observations_data_list:
                    df_obs = pd.DataFrame(observations_data_list)
                    obs_excel_filename = os.path.join(log_dir, f"episode_{timestep}_observations.xlsx")
                    df_obs.to_excel(obs_excel_filename, index=False)
                    print(f"[INFO] Observations data saved to: {obs_excel_filename}")
                
                # Save actions and errors data
                if actions_data_list:
                    df_actions = pd.DataFrame(actions_data_list)
                    actions_excel_filename = os.path.join(log_dir, f"episode_{timestep}_actions_errors.xlsx")
                    df_actions.to_excel(actions_excel_filename, index=False)
                    print(f"[INFO] Actions and errors data saved to: {actions_excel_filename}")
            
            # Reset buffers for next episode
            insertion_real_moves = []
            insertion_processed = []
            observations_data_list = []  # Reset observations data
            actions_data_list = []  # Reset actions data
            # timestep = 0
            break
        else:
            timestep += 1
...
def main():
    ...
    previous_actions = torch.zeros((1, 6), device=agent_cfg.device, dtype=torch.float32)
    # MANUAL_RESET_POS = torch.tensor([[01.07730, -0.44910, 0.08000, 0.90000, 0.10000, 0.10000]], device=agent_cfg.device)
    MANUAL_RESET_POS = torch.tensor([[ 0.8836, -0.3286, 0.1045, 0.9168, 0.3883, 0.4726]], device=agent_cfg.device) # LIFT
    zero_vel = torch.zeros_like(MANUAL_RESET_POS)
    robot_asset.set_joint_position_target(MANUAL_RESET_POS, joint_ids=slice(0, 6))
    robot_asset.write_joint_state_to_sim(MANUAL_RESET_POS, zero_vel, joint_ids=slice(0, 6))
    robot_asset.write_data_to_sim()
