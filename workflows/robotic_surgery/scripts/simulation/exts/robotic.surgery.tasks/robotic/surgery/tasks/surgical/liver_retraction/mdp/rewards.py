...
def gallbladder_mask_tensor(image_tensor):
    ...
    # 1. Exclude gray pixels (difference between channels must be > 5)
    diff_gr = torch.abs(g - r)
    diff_gb = torch.abs(g - b)
    is_not_gray = (diff_gr > 5) | (diff_gb > 5)

    # # 2. Color range thresholds for gallbladder detection
    # mask_range = (r >= 5) & (r <= 110) & \ 
    #              (g >= 15) & (g <= 125) & \
    #              (b >= 3)  & (b <= 95)

    # 2. Color range thresholds for gallbladder detection
    mask_range = (r >= 5) & (r <= 140) & \
                 (g >= 15) & (g <= 160) & \
                 (b >= 3)  & (b <= 120)

    # 3. Green channel dominance requirement
    green_dominant = (g > r) & (g > b)

    # Combine all conditions into final mask (True/False)
    final_mask = is_not_gray & mask_range & green_dominant
    return final_mask
...
def visual_exposure_reward(env):
    ...
    if not hasattr(visual_exposure_reward, 'warmup_step'):
        visual_exposure_reward.warmup_step = torch.zeros(env.scene.num_envs, device=env.device, dtype=torch.long)
        visual_exposure_reward.warmup_duration = 30
